...
as_schedule <- function(data, treatment, response, append = TRUE, outcome_prefix = "Y") {
  ...
  if (!all(c(treatment, response) %in% colnames(data))) {
  ...
  # Store attributes
  attr(schedule, "treatment") <- treatment
  attr(schedule, "response") <- response
  attr(schedule, "treatment_levels") <- levels(data[[treatment]])
  attr(schedule, "potential_outcome_cols") <- paste0(outcome_prefix, levels(data[[treatment]]))
